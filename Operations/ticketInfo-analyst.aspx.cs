...
public partial class Operations_Default : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            int i = 0;
            DataRow dr;
            SqlDataAdapter sda = new SqlDataAdapter("select TMSTicketID,TMSTicketSubject,TMSTicketDescription,TMSTicketPriority,TMSTicketDate,TMSHostName,TMSTicketStatus,EmployeeID,TMSTicketOwnership,TMSTicketLoaction from TMSTicketMaster where TMSTicketID = '" + Session["TicketID"] + "'", con);
            sda.Fill(dt);
            if (dt.Rows.Count > 0)
            {
                dr = dt.Rows[i];
    ...
    protected void MailToUser_Click1(object sender, EventArgs e)
    {
    ...
    protected void Update_Ticket_Status_Click(object sender, EventArgs e)
    {

        string conString1 = ConfigurationManager.ConnectionStrings["SNPLDBSTRING"].ConnectionString;
        SqlConnection con = new SqlConnection(conString1);
       
        SqlCommand com = new SqlCommand("update TMSTicketMaster set TMSTicketStatus=@status where TMSTicketID=@ticid", con);
        com.Parameters.AddWithValue("@status", DropDownList1.SelectedValue);
        com.Parameters.AddWithValue("@ticid", Session["TicketID"]);
        con.Open();
        com.ExecuteNonQuery();
        //SendMail();
        con.Close();
    ...
    protected void ChangeDepartment_Click(object sender, EventArgs e)
    {
        string conString1 = ConfigurationManager.ConnectionStrings["SNPLDBSTRING"].ConnectionString;
        SqlConnection con = new SqlConnection(conString1);
        SqlCommand com = new SqlCommand("update TMSTicketMaster set [TMSTicketDepartment]=@dept,[TMSTicketOwnership] = null where TMSTicketID=@ticid", con);
        com.Parameters.AddWithValue("@dept", DropDownList2.SelectedValue);
        com.Parameters.AddWithValue("@ticid", Session["TicketID"]);
        con.Open();
        com.ExecuteNonQuery();
        //SendMail();
